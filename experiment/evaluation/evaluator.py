...
class Evaluator:
    ...
    def evaluate_stealthiness(self, original_image, itfgsm_image, mt_ifgsm_image):
        ...
        hash_mt_ifgsm = imagehash.phash(self.tensor_to_pil(mt_ifgsm_image))

        original_itfgsm_distance = hash_original - hash_itfgsm
        original_mt_ifgsm_distance = hash_original - hash_mt_ifgsm

        original_image = np.array(original_image)
        itfgsm_image = np.array(itfgsm_image.detach().numpy())
        mt_ifgsm_image = np.array(mt_ifgsm_image.detach().numpy())

        l2_distance_itfgsm = np.linalg.norm(original_image - itfgsm_image).round(2)
        l2_distance_mt_ifgsm = np.linalg.norm(original_image - mt_ifgsm_image).round(2)

        itfgsm_image = itfgsm_image.squeeze(0)
        mt_ifgsm_image = mt_ifgsm_image.squeeze(0)
        original_image_last = original_image.transpose(1, 2, 0)
        itfgsm_image_last = itfgsm_image.transpose(1, 2, 0)
        mt_ifgsm_image_last = mt_ifgsm_image.transpose(1, 2, 0)

        ssim_itfgsm = ssim(original_image_last, itfgsm_image_last, channel_axis=-1, data_range=1.0)
        ssim_mt_ifgsm = ssim(original_image_last, mt_ifgsm_image_last, channel_axis=-1, data_range=1.0)

        if self.log_values:
            print("###################### Hash Sensitive Difference ######################")
            print("Original and ITFGSM Image Hash Sensitive Difference:", original_itfgsm_distance)
            print("Original and MT-IFGSM Image Hash Sensitive Difference:", original_mt_ifgsm_distance)
            print("############################# L2 Distance #############################")
            print("L2 Distance between Original and ITFGSM Image:", l2_distance_itfgsm)
            print("L2 Distance between Original and MT-IFGSM Image:", l2_distance_mt_ifgsm)
            print("############################# SSIM #############################")
            print("SSIM between Original and ITFGSM Image:", ssim_itfgsm)
            print("SSIM between Original and MT-IFGSM Image:", ssim_mt_ifgsm)

        return original_itfgsm_distance, original_mt_ifgsm_distance, l2_distance_itfgsm, l2_distance_mt_ifgsm, ssim_itfgsm, ssim_mt_ifgsm
